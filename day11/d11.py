...
def chg(seat, neighbors):
    if seat == EMPTY and all([n == EMPTY or n == "." for n in neighbors]):
        return OCCUP
    if seat == OCCUP and sum([int(n == OCCUP) for n in neighbors]) >= 4:
        return EMPTY
    return seat

def get_neighbors(i,j,layout):
...
def step(prev_layout, next_layout):
    x_len, y_len = prev_layout.shape
    for i in range(x_len):
        for j in range(y_len):
            if prev_layout[i, j] == FLOOR:
                continue
            neighbors = get_neighbors(i,j,prev_layout)
...
with open("input.txt", "r") as f:
    ...
    print((layout1 == OCCUP).astype(int).sum())
